...
function isHotTask(createdAt: string): boolean {
  const createdTs = new Date(createdAt).getTime()
  if (Number.isNaN(createdTs)) return false
  return Date.now() - createdTs <= HOT_WINDOW_MS
}
  ...
  useEffect(() => {
    ...
    const loadTasks = async () => {
      const { data, error } = await supabase
        .from('tasks')
        .select('id, title, budget, created_at, status')
        .eq('status', 'open')
        .not('budget', 'is', null)
        .gt('budget', 0)
          ...
          <div className="min-w-0 flex-1 overflow-hidden">
            ...
            {marqueeItems.map((task, index) => {
              ...
              return (
                ...
                >
                  {isHotTask(task.created_at) && <span aria-hidden="true">🔥</span>}
