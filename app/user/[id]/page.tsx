...
function UserProfileContent() {
  ...
  useEffect(() => {
    if (!profile || userRatings.length === 0) return
  ...
  const loadUserProfile = async () => {
    try {
      ...
      // Check if userId is a UUID (36 chars with hyphens) or a slug
      const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId)
      
      // Load profile by ID or slug (for both helpers and non-helpers)
      let profileData = null
      let profileError = null
      
      if (isUUID) {
        // If it's a UUID, query by ID directly (works for all users, helpers and taskers)
        ...
        if (error) {
          console.error('📋 User Profile: Error loading by ID:', error)
          // Try by profile_slug as fallback
          ...
          if (slugData && !slugError) {
            ...
            console.log('📋 User Profile: Found profile by slug fallback')
          } else {
            profileData = null
            profileError = error // Use original error
          }
        } else {
          profileData = data
          console.log('📋 User Profile: Found profile by ID')
        }
      } else {
        // If it's a slug, query by slug first, then try ID as fallback
        const { data, error } = await supabase
          .from('profiles')
          .select('*')
          .eq('profile_slug', userId)
        ...
        if (error || !data) {
          console.log('📋 User Profile: Not found by slug, trying as UUID...')
          // Try as UUID if slug lookup fails
          ...
          if (uuidMatch) {
            ...
            if (idData && !idError) {
              profileData = idData
              console.log('📋 User Profile: Found profile by ID fallback')
            } else {
              profileData = null
              profileError = idError || error
            }
          } else {
            profileData = data
            profileError = error
      ...
      if (profileError || !profileData) {
        console.error('📋 User Profile: Error loading profile:', {
          userId,
          isUUID,
          error: profileError,
      ...
      // Log full profile data to see what's actually loaded
      console.log('📋 User Profile: FULL profile data object:', JSON.stringify(profileData, null, 2))
      console.log('📋 User Profile: Summary:', {
        id: profileData.id,
        full_name: profileData.full_name,
        email: profileData.email,
        is_helper: profileData.is_helper,
        is_tasker: profileData.is_tasker,
        profile_slug: profileData.profile_slug,
        bio: profileData.bio ? `"${profileData.bio.substring(0, 50)}..."` : 'null/empty',
        bio_length: profileData.bio?.length || 0,
        skills: profileData.skills ? `[${profileData.skills.length} items]` : 'null/empty',
        services_offered: profileData.services_offered ? `[${profileData.services_offered.length} items]` : 'null/empty',
        professional_offerings: profileData.professional_offerings ? `[${profileData.professional_offerings.length} items]` : 'null/empty',
        professions: profileData.professions ? `[${profileData.professions.length} items]` : 'null/empty',
        qualifications: profileData.qualifications ? `[${profileData.qualifications.length} items]` : 'null/empty',
        badges: profileData.badges ? `[${profileData.badges.length} items]` : 'null/empty',
        missing_fields: missingFields,
        all_keys: Object.keys(profileData).sort()
      })
      
      if (missingFields.length > 0) {
        console.warn('⚠️ User Profile: Some expected fields are missing from query result:', missingFields)
      }
      ...
      if (reviewsData && reviewsData.length > 0) {
        ...
        // Calculate overall average from all reviews (for backward compatibility)
      ...
      // For helpers: tasks they completed (assigned_to)
      // For taskers: tasks they posted that were completed (created_by)
      const tasksQuery = profileData.is_helper
        ? supabase
            .from('tasks')
            .select(`
              *,
      ...
      const profileUrl = `${window.location.origin}/user/${profileData.profile_slug || profileData.id}`
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">{error || 'This profile does not exist or is not available.'}</p>
          <Link
            href="/tasks"
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-wrap gap-2 justify-start sm:justify-end mt-4 sm:mt-0">
                  {profile.is_helper && (!currentUserId || currentUserId !== profile.id) && (
                    <button
                      onClick={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Completed Tasks - Show for helpers (tasks they completed) or taskers (tasks they posted) */}
            {completedTasks.length > 0 && (
              <div className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">
                  {profile.is_helper ? 'Completed Tasks' : 'Tasks Posted'}
                </h2>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-4">
                          ...
                          {/* Rating Display */}
                          {taskRating ? (
                            <div className="flex items-center gap-1.5 sm:gap-2 mt-2 pt-2 border-t border-gray-100 flex-wrap">
                            ...
                            <div className="mt-2 pt-2 border-t border-gray-100">
                              <span className="text-xs sm:text-sm text-gray-500 italic">No reviews yet</span>
                            </div>
                          )}
                        </div>
                      </Link>
